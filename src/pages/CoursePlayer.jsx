      ...
      <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.125rem' }}>
          {course.moduleList.map((m, i) => {
            ...
            return (
              <Link key={m.id} to={`/learn/${courseId}/module/${m.id}`} style={{ textDecoration: 'none' }} onClick={() => isMobile && setSidebarOpen(false)}>
                <div style={{
                  ...
                  <div style={{ flexShrink: 0, color: done ? '#10b981' : active ? '#34d399' : '#4a5a78' }}>
          ...
          <Link to={`/learn/${courseId}/assessment`} style={{ textDecoration: 'none' }} onClick={() => isMobile && setSidebarOpen(false)}>
            <div style={{ padding: '0.625rem 1.25rem', display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
              <Award size={15} color={progress?.assessmentPassed ? '#10b981' : '#4a5a78'} />
  ...
  return (
    <div style={{ background: '#0a0e1a', minHeight: '100vh', color: '#f0f4f8' }}>
      ...
      {isMobile && sidebarOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 49, backdropFilter: 'blur(2px)' }} onClick={() => setSidebarOpen(false)} />
      )}

      {!isMobile && (
        <aside style={{
          width: '260px', background: '#0d1526', borderRight: '1px solid rgba(255,255,255,0.07)',
          padding: '1.5rem 0', position: 'fixed', top: '64px', left: 0,
          height: 'calc(100vh - 64px)', overflowY: 'auto', zIndex: 10,
        }}>
          <SidebarContent />
        </aside>
      )}
      ...
      {isMobile && (
        <aside ref={drawerRef} style={{
          width: '280px', background: '#0d1526', borderRight: '1px solid rgba(255,255,255,0.1)',
          padding: '1.5rem 0', position: 'fixed', top: 0, left: 0,
          height: '100vh', overflowY: 'auto', zIndex: 50,
          transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)',
          transition: 'transform 0.25s ease', paddingTop: '4rem',
        }}>
          <SidebarContent />
        </aside>
      )}
        ...
        {isMobile && (
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '0.68rem', color: '#10b981', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Module {moduleIndex + 1} of {course.moduleList.length}
              </div>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#f0f4f8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {module.title}
              </div>
            </div>
          </div>
        ...
        <div style={{ maxWidth: '720px' }}>
          {!isMobile && (
            <div style={{ marginBottom: '2rem', paddingBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
              <div style={{ color: '#10b981', marginBottom: '0.375rem', fontSize: '0.62rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'monospace' }}>
                Module {moduleIndex + 1} of {course.moduleList.length}
              </div>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '0.75rem' }}>
                ...
                {currentModuleDone && (
                  <span style={{ display: 'flex', gap: '0.375rem', alignItems: 'center', color: '#10b981', fontSize: '0.8rem', fontWeight: 600 }}>
          ...
          {quiz.length > 0 && (
            <div style={{
              background: '#0d1526', border: '1px solid rgba(16,185,129,0.15)',
              ...
              {quiz.map((q, qi) => {
                ...
                return (
                  <div key={q.id} style={{
                    marginBottom: '2rem', paddingBottom: qi < quiz.length - 1 ? '2rem' : 0,
                    borderBottom: qi < quiz.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none',
                  }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                      <span style={{ fontSize: '0.68rem', fontWeight: 700, color: '#10b981', letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: 'monospace' }}>
                        Question {qi + 1} of {quiz.length}
                      </span>
                      <div style={{ display: 'flex', gap: '3px' }}>
                        {quiz.map((_, idx) => (
                          <div key={idx} style={{ width: '18px', height: '3px', borderRadius: '2px', background: idx <= qi ? '#10b981' : 'rgba(255,255,255,0.1)' }} />
                        ))}
                      </div>
                    </div>

                    <p style={{ color: '#f1f5f9', fontWeight: 600, fontSize: '0.95rem', marginBottom: '1rem', lineHeight: 1.65 }}>{q.question}</p>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem', marginBottom: '0.875rem' }}>
                      {q.options.map((opt, oi) => {
                        ...
                        if (!quizSubmitted && isSelected) {
                          bg = 'rgba(16,185,129,0.12)'; border = '1px solid rgba(16,185,129,0.5)';
                          color = '#f1f5f9'; badgeBg = 'rgba(16,185,129,0.2)'; badgeColor = '#34d399';
                        }
                        if (quizSubmitted) {
                          if (isThisCorrect) {
                            bg = 'rgba(34,197,94,0.1)'; border = '1px solid rgba(34,197,94,0.45)';
                            color = '#86efac'; badgeBg = 'rgba(34,197,94,0.2)'; badgeColor = '#4ade80'; icon = '\u2713';
                          } else if (isSelected) {
                            bg = 'rgba(239,68,68,0.1)'; border = '1px solid rgba(239,68,68,0.45)';
                            color = '#fca5a5'; badgeBg = 'rgba(239,68,68,0.2)'; badgeColor = '#f87171'; icon = '\u2717';
                          }
                        }

                        return (
                          <button key={oi} onClick={() => { if (!quizSubmitted && !isAnswered) setQuizAnswers(prev => ({ ...prev, [q.id]: oi })); }}
                            disabled={quizSubmitted || isAnswered}
                            style={{
                              background: bg, border, borderRadius: '10px', padding: '0.875rem 1rem', color,
                              textAlign: 'left', cursor: (quizSubmitted || isAnswered) ? 'default' : 'pointer',
                              transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', gap: '0.875rem',
                          ...
                          >
                            <span style={{
                              ...
                              fontSize: '0.75rem', fontWeight: 700, flexShrink: 0, color: badgeColor, fontFamily: 'monospace',
                            }}>{icon}</span>
                    ...
                    {isAnswered && !quizSubmitted && (
                      <div style={{
                        ...
                        {q.explanation && <div style={{ fontSize: '0.8rem', color: '#94a3b8', lineHeight: 1.6 }}>{q.explanation}</div>}
                      </div>
                    )}
              ...
              {!quizSubmitted ? (
                <button className="btn-primary" disabled={!quizAllAnswered} onClick={() => setQuizSubmitted(true)}
          ...
          <div style={{
            ...
            <button className="btn-primary" onClick={handleNext} disabled={quiz.length > 0 && !quizSubmitted}
